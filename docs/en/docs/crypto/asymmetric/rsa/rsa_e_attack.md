...
e is particularly small, for example e is 3.

### Attack Principle

Assume the user uses a key with $e=3$. Considering that the encryption relationship satisfies:
...
The attacker can enumerate $k$ from small to large, taking the cube root each time, until an integer result is obtained.

### Example

Here we use the XMan Summer Camp Session 1 classroom exercise as an example (reproduced on Jarvis OJ). The attachment contains a `flag.enc` and `pubkey.pem`, which are obviously the ciphertext and public key. Let's first read the public key with `openssl`.
...
Seeing $e=3$, this is clearly a small public exponent attack. Here we use the Crypto library to read the public key, and use multiprocessing to speed up the cracking process.
...
The brute force takes quite a while. Got the flag
...
## RSA Derived Algorithm — Rabin Algorithm

### Attack Conditions

The characteristic of the Rabin algorithm is $e=2$.
...
- Use the Extended Euclidean Algorithm to compute $y_p$ and $y_q$:

$$
y_p \cdot p + y_q \cdot q = 1
$$

- Solve for the four possible plaintexts:
...
In general, $p \equiv q \equiv 3 \pmod 4$ is satisfied. For cases where it is not, please refer to the corresponding algorithms for a solution.

### Example

Here we use the XMan Summer Camp Session 1 classroom exercise (reproduced on Jarvis OJ) as an example. Let's read the public key.
...
$e=2$, consider the Rabin algorithm. First, let's factor p and q to get
