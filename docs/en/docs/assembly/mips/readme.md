...
Here we use MIPS32 as the basis for introduction.

## 0x01 Registers

###  (1) General-Purpose Registers

MIPS has 32 general-purpose registers (GPRs), denoted with a dollar sign (`$`). They can be represented as `$0~$31`, or by register names such as `$sp`, `$t9`, `$fp`, etc.

| Register Number | Conventional Name | Usage                                                  | Usage                                                     |
| --------------- | ----------------- | ------------------------------------------------------ | --------------------------------------------------------- |
| $0              | $zero             | Hard-wired to 0                                        |                                                           |
| $1              | $at               | Reserved for pseudo-instructions                       |                                                           |
| $2 - $3         | $v0, $v1          | Return values from functions                           | Stores expression or function return values                                  |
| $4 - $7         | $a0 - $a3         | Arguments to functions - not preserved by subprograms  | Function arguments                                                |
| $8 - $15        | $t0 - $t7         | Temporary data, not preserved by subprograms           |                                                           |
| $16 - $23       | $s0 - $s7         | Saved registers, preserved by subprograms              |                                                           |
| $24 - $25       | $t8 - $t9         | More temporary registers, not preserved by subprograms | Temporary registers, supplementary to $t0 - $t7; $t9 is often related to function calls |
| $26 - $27       | $k0 - $k1         | Reserved for kernel. Do not use.                       |                                                           |
| $28             | $gp               | Global Area Pointer (base of global data segment)      |                                                           |



MIPS is a load-store architecture, which means that to access memory we must use load and store instructions. All other instructions (add, sub, mul, div, etc.) must obtain their operands from registers and store their results in registers. For example:

```
    sum = x + y
```

Assuming sum, x, and y are variables in the program, their MIPS assembly representation is:
...
The MIPS32 architecture also defines 3 special registers: PC (Program Counter), HI (high-order word of multiply/divide result register), and LO (low-order word of multiply/divide result register). During multiplication, HI and LO store the multiplication result, where HI stores the upper 32 bits and LO stores the lower 32 bits. During division, HI stores the remainder and LO stores the quotient.
